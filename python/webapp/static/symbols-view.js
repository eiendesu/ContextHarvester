...
(function () {
  ...
  function getFiltered() {
    ...
    return catalog.filter((s) => {
      ...
      if (!q) return true;
      const hay = `${s.label} ${s.qualifiedName} ${s.filePath} ${s.type} ${s.typeLabel || ''}`.toLowerCase();
  ...
  function updateMeta(filtered) {
    ...
    const files = window._symFileCount != null ? ` · ${window._symFileCount} file indicizzati` : '';
    el.textContent =
      `Catalogo: ${catalog.length} simboli${files}${src} · mostrati ${filtered.length}${suffix} — ${parts.join(', ')}`;
  }
