...
(function () {
  ...
  function mockUrl(path) {
    for (const [prefix, mock] of Object.entries(MOCK_MAP)) {
      if (path === prefix) return mock;
    }
    if (path.startsWith("/api/graph/file"))
      return "/static/mock/graph-file.json";
    if (path.startsWith("/api/graph/detail"))
      return "/static/mock/graph-detail.json";
    if (path.startsWith("/api/graph/search"))
      return "/static/mock/graph-search.json";
    if (path.startsWith("/api/graph/expand"))
      return "/static/mock/graph-expand.json";
    if (path.startsWith("/api/graph/impact"))
      return "/static/mock/graph-impact.json";
    if (path.startsWith("/api/graph/api-links"))
      return "/static/mock/graph-api-links.json";
    if (path.startsWith("/api/graph/path"))
      return "/static/mock/graph-path.json";
    return null;
  }
  ...
  function initNetwork() {
    ...
    network.on("click", (params) => {
      if (params.nodes.length) {
        ...
        $("graph-selection").textContent =
          `Selezionato: ${n.label} (${selectedNodeId})`;
  ...
  function applyGraphFilterAndFocus() {
    ...
    if (matching.length && network) {
      ...
      $("graph-selection").textContent =
        `${matching.length} nodi corrispondenti al filtro`;
    } else if (network) {
  ...
  $("impact-run").addEventListener("click", async () => {
    ...
    if (data.crossLayer)
      html +=
        '<p class="muted">Modalità cross-layer (solo edge API/import)</p>';
    for (const [d, items] of Object.entries(data.impact || {})) {
      html += `<h4>Distanza ${d}</h4><ul>`;
      items.forEach((it) => {
        const typeBadge = it.type
...
<p>Best-effort: non copre GraphQL, client HTTP custom o route dinamiche complesse. Utile per tracciare “chi chiama quale API”.</p>`,
  ...
  document.addEventListener("keydown", (e) => {
    if (
      e.key === "Escape" &&
      !$("analysis-info-modal").classList.contains("hidden")
    ) {
      closeAnalysisHelp();
    }
  });
  ...
  async function loadAnalysis(recalc) {
    ...
    $("analysis-meta").textContent = data.analyzedAt
      ? `Ultima: ${data.analyzedAt}`
      : "";
    const sections = [
      {
        id: "deadCode",
        ...
        items: data.deadCode,
        fmt: (x) =>
          `${escapeHtml(x.label)} — <code>${escapeHtml(x.file)}</code>`,
      },
      {
        id: "circularDeps",
        title: "🔵 Dipendenze circolari",
      ...
      },
      {
        ...
        items: data.hotspots,
        fmt: (x) =>
          `${escapeHtml(x.label)} — score ${x.score} (${x.recentCommits} commit)`,
      },
      {
        id: "testGap",
        title: "⚪ Test gap",
        items: data.testGap,
        fmt: (x) => escapeHtml(x.label),
      },
      {
        id: "similarFunctions",
        ...
        fmt: (x) =>
          `${escapeHtml(x.f1)} ↔ ${escapeHtml(x.f2)} (${x.shared} condivisi, ${x.similarity})`,
      },
      {
        id: "apiEdges",
        ...
        items: data.apiEdges,
        fmt: (x) =>
          `${escapeHtml(x.from)} → ${escapeHtml(x.to)} (${escapeHtml(x.api)})`,
      },
    ];
    let html = "";
    ...
    $("analysis-sections")
      .querySelectorAll(".analysis-info-btn")
      .forEach((btn) => {
  ...
  async function loadFunctions() {
    ...
    window._funcCatalog = fmap.functions || [];
    const filter = ($("fn-filter").value || "").toLowerCase();
    const funcs = window._funcCatalog
    ...
    document.querySelectorAll(".fn-row").forEach((row) => {
      row.addEventListener("click", () => {
        const func = (window._funcCatalog || []).find(
          (f) => f.id === row.dataset.id,
        );
        document.querySelector('.tab[data-tab="sigma"]').click();
        if (func && window.chSigmaView?.showFunction) {
          window.chSigmaView.showFunction(func);
        }
      });
    });
  }
  ...
  $("graph-lf-search")?.addEventListener("click", () =>
  ...
  async function runLabelFirstSearch(source) {
    ...
    if (!input) {
      statusEl.textContent =
        "Inserisci una descrizione (es. pagina lista contratti)";
      return;
    }
    if (!fromGraph) {
    ...
    try {
      ...
      ws.onopen = () => {
        ws.send(
          JSON.stringify({ action: "label_first", input, depth, maxNodes }),
        );
      };
      ws.onmessage = (ev) => {
