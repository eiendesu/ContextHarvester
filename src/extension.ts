...
import {
  buildConfig,
  getAutoIndexSettings,
  getRepoPath,
  isPathExcluded,
} from "./settings";
import { openGraphView, openGraphViewEmbedded } from "./graphView";
...
export async function activate(
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(
      ContextHarvesterPanel.viewType,
      panelProvider,
    ),
    vscode.window.registerWebviewViewProvider(
      DevLabPanel.viewType,
      devLabProvider,
    ),
    vscode.window.registerWebviewViewProvider(
      RoslynPanel.viewType,
      roslynProvider,
    ),
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("context-harvester.openPanel", async () => {
      await vscode.commands.executeCommand(
        "workbench.view.extension.context-harvester",
      );
    }),
    vscode.commands.registerCommand(
      "context-harvester.openDevLab",
      async () => {
        await vscode.commands.executeCommand(
          "workbench.view.extension.context-harvester",
        );
    ...
    vscode.commands.registerCommand("context-harvester.runRoslynScan", () =>
      roslynProvider.runScan(),
    ),
    vscode.commands.registerCommand(
      "context-harvester.rebuildIndex",
      async () => {
        await rebuildIndex(panelProvider);
        await roslynProvider.refreshState();
      },
    ),
    vscode.commands.registerCommand("context-harvester.generateContext", () =>
      generateContext(panelProvider),
    ),
    vscode.commands.registerCommand("context-harvester.openLastContext", () =>
      openLastContext(context),
    ),
    vscode.commands.registerCommand(
      "context-harvester.checkOllama",
      async () => {
        const profile = await getActiveProfile(context);
        const result = await checkOllamaForProfile(profile);
        if (!result.reachable) {
          vscode.window.showErrorMessage(
            `Ollama non raggiungibile: ${result.urls.join(", ")}`,
          );
          ...
          vscode.window.showInformationMessage(
            "Ollama OK — modelli del profilo attivo presenti.",
          );
        }
        await panelProvider.refreshState();
      },
    ),
    vscode.commands.registerCommand("context-harvester.resetIndex", () =>
      panelProvider.resetIndex(),
    ),
    vscode.commands.registerCommand(
      "context-harvester.showExecutionLog",
      () => {
    ...
    vscode.commands.registerCommand(
      "context-harvester.openKnowledgeGraph",
      () => {
        return openGraphViewEmbedded(context);
      },
    ),
    vscode.commands.registerCommand(
      "context-harvester.validateCommunities",
      () => {
...
async function setupMcpAutoStart(
  context: vscode.ExtensionContext,
): Promise<void> {
  const cfg = vscode.workspace.getConfiguration("contextHarvester");
  if (
    !cfg.get<boolean>("enableMcpServer", false) ||
    !cfg.get<boolean>("mcp.autoStart", false)
  ) {
    return;
  }
  try {
...
function setupAutoIndex(context: vscode.ExtensionContext): void {
  const triggerIncremental = () => {
    ...
    void panelProvider.runAction("incremental_index", {
      ...buildConfig(),
      incremental: true,
    });
  };
  ...
  context.subscriptions.push(
    vscode.workspace.onDidSaveTextDocument((doc) => {
      ...
      if (
        !repo ||
        !doc.uri.fsPath.startsWith(repo) ||
        isPathExcluded(doc.uri.fsPath, repo)
      ) {
        return;
      }
      debouncedSave();
  ...
  function setupAutoIndexInterval() {
    ...
    if (autoIndex) {
      autoIndexTimer = setInterval(
        triggerIncremental,
        autoIndexIntervalMinutes * 60 * 1000,
      );
    }
  }
}
